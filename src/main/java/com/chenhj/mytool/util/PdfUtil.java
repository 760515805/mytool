...
public class PdfUtil {
	...
	public static String pdf2Image(File PdfFilePath, String dstImgFolder, int dpi) throws IOException {
		...
		try {
			...
			if (!createDirectory(dstImgFolder))
				throw new IOException(dstImgFolder + "创建失败...");
	...
	public static String toPdf(List<File> files, String basicPath) throws Exception {
		...
		try {
			String pdfFilePathPrefix = basicPath + File.separator + DateUtils.getNowTime("yyyyMMdd") + File.separator
					+ "img2pdf" + File.separator;
			...
			for (File file1 : files) {
				if (file1.getName().toLowerCase().endsWith(".png") || file1.getName().toLowerCase().endsWith(".jpg")
